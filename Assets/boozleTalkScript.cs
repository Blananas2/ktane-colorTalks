...
public class boozleTalkScript : MonoBehaviour
{
    ...
    string[] wordlists = {
        ...
        "AZURE,BLACK,BLUE,BROWN,CYAN,FUCHSIA,GRAY,GREEN,MAGENTA,PINK,PURPLE,RED,VIOLET,WHITE,YELLOW",
...
#pragma warning disable 414
    private readonly string TwitchHelpMessage = "!{0} submit 2 [Presses the module when the timer contains a 2.] | \"submit\" is optional.";
#pragma warning restore 414

    IEnumerator ProcessTwitchCommand(string command)
    {
        command = command.Trim().ToLowerInvariant();
        var m = Regex.Match(command, @"^\s*(?:(submit|press|submit\s+at|press\s+at)\s+)?(?<num>\d)\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        if (!m.Success)
            yield break;

        yield return null;
        while (!Bomb.GetFormattedTime().Contains(m.Groups["num"].Value))
